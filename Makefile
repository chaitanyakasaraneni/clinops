# Makefile for clinops
# Keeps the version in sync across pyproject.toml and the clinops/__init__.py
# fallback in a single run. Portable to GNU Make 3.81 (macOS default) — recipes
# are single POSIX-sh lines.

PKG       := clinops
PYPROJECT := pyproject.toml
INIT      := clinops/__init__.py
...
help:
	@echo "clinops — current version: $(CURRENT_VERSION)"
	@echo ""
	@echo "Version bumping (syncs $(PYPROJECT), $(INIT); rolls $(CHANGELOG) if present):"
...
_bump:
	...
	sed -E 's/__version__ = "[0-9]+\.[0-9]+\.[0-9]+-dev"/__version__ = "'"$$new"'-dev"/' $(INIT) > $(INIT).tmp && mv $(INIT).tmp $(INIT); \
	cl_note=""; \
	...
	echo "Bumped $$cur -> $$new  (date: $$today)"; \
	echo "  updated: $(PYPROJECT), $(INIT)$$cl_note"; \
	echo "Next: review changes, then: git commit -am \"release v$$new\" && make tag && git push --follow-tags"
